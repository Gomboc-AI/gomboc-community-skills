...
description: >-
  ...
  cache, then orl rules pull. Depends on: gomboc_community_know_orl_runtime_resolution,
  gomboc_community_cap_orl_rules_pull.
---

# Task: Resolve Existing Rules

Remote discovery via `orl` is a **pull** (`orl rules pull --search`). There is no search-only `orl` command.
...
3. **Pull** — invoke **`gomboc_community_cap_orl_rules_pull`** with a compound `--search` query into `cache_dir`; refresh cache metadata.
4. **Report** match quality: existing / partial / none.
